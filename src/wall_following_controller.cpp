    ...
    const Eigen::Isometry3d& pose)
{
    ...
    std::sort(in_range_detections.begin(), in_range_detections.end(),
    ...
    for (const auto* detection : in_range_detections)
    {
        ...
        if (detection->distance < config_.min_wall_distance)
        {
            ...
            RCLCPP_DEBUG_THROTTLE(logger_, steady_clock, 1000,
                ...
                detection->angle, detection->angle * 180.0 / M_PI, detection->distance);
            break;
        }
    }
    ...
    {
        RCLCPP_INFO_THROTTLE(logger_, steady_clock, 1000,
            "Target detection found: angle=%.3f rad (%.1f°), distance=%.3f m, base_link_pos=[%.3f, %.3f]",
            nearest->angle, nearest->angle * 180.0 / M_PI,
            nearest->distance, nearest->x(), nearest->y());

        // 1. Create transform for detection in base_link frame
        const Eigen::Isometry3d base_link_T_detection = std::invoke([&nearest]{
        ...
        RCLCPP_DEBUG_STREAM_THROTTLE(logger_, steady_clock, 1000,
...
ControlInput WallFollowingController::update(const SystemResponse& input)
{
    ...
    if (target_point_odom_.has_value())
    {
        ...
        RCLCPP_INFO_THROTTLE(logger_, steady_clock, 500,
            ...
            distance_error, config_.min_wall_distance, distance_to_target,
            angle_error, angle_error * 180.0 / M_PI,
            within_min_distance, reached_min_distance_,
            (!config_.continuous && !reached_min_distance_) ? "APPROACH" :
            (!config_.continuous && reached_min_distance_) ? "ALIGN" : "FOLLOW");
        ...
        if (!config_.continuous)
        {
            ...
            else
            {
                ...
                {
                    // Apply rotation
                    double angular_velocity = (angle_error > 0) ?
        ...
        else
        {
            ...
            if (std::abs(angle_error) > config_.wall_alignment_tolerance)
            {
                angular_velocity = (angle_error > 0) ?
                    config_.angular_speed : -config_.angular_speed;
                RCLCPP_DEBUG_THROTTLE(logger_, steady_clock, 500,
                    "Continuous mode: Applying rotation (angle_error=%.3f rad > tolerance=%.3f rad)",
                    std::abs(angle_error), config_.wall_alignment_tolerance);
            }
            else
            {
                RCLCPP_DEBUG_THROTTLE(logger_, steady_clock, 500,
    ...
    else
    {
        ...
        RCLCPP_WARN_THROTTLE(logger_, steady_clock, 1000,
            ...
            input.detections.size(), config_.max_detection_range);
        output.cmd_vel.linear.x = 0.0;
        output.cmd_vel.angular.z = 0.0;
    }
    ...
    RCLCPP_INFO_THROTTLE(logger_, steady_clock, 500,
        "=== CONTROLLER OUTPUT === cmd_vel: linear.x=%.3f m/s, angular.z=%.3f rad/s (%.1f°/s), is_complete=%d",
        output.cmd_vel.linear.x,
        output.cmd_vel.angular.z, output.cmd_vel.angular.z * 180.0 / M_PI,
        output.is_complete);
